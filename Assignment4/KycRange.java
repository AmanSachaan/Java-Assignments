...
public class KycRange {
    ...
    public void readDates() {
        System.out.println("Please Enter the signup date and current date in dd-MM-yyyy by including a space in between ");
    ...
    public void printKycRanges(Calendar signup,Calendar current, int currentYear) {
    ...
    public static void main(String args[]) throws NumberFormatException, NoSuchElementException{
        System.out.println("Enter the number of KYC ranges need to be finded : ");
        int n = scanner.nextInt();
        scanner.nextLine();
        KycRange kycRange;
        for (int i = 0; i < n; i++) {
            kycRange=new KycRange();
            kycRange.readDates();
            kycRange.printKycRanges(kycRange.signup,kycRange.current,kycRange.currentYear);
        }
